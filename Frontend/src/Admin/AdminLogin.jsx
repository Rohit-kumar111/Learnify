...
const AdminLogin = () => {
   ...
   async function formHandler(e){
      ...
      try {
         ...
         const response= await axios.post(`${BACKEND_URL}/admin/login`,{// sending the data
          email,
          password
         },{
         ...
         localStorage.setItem("admin",JSON.stringify(response.data))//STORING IN LOCALSTORAGE and coverting it into a string
          ...
          <div className='h-screen flex items-center justify-center text-white max-w-[75vw] mx-auto '>
            ...
            <nav className=' absolute top-0 left-0 w-full flex items-center justify-between py-6 '>
              <div className='flex space-x-2 ml-4' >
                <img src="/course_logo.jpg" alt="" className='w-[35px] h-[35px] rounded-full' />
              ...
              <div className='space-x-2 sm:space-x-2 sm:right-6 relative right-3 '>
                <Link to={"/admin/signup"} className='border-2 bg-transparent border-white px-4 py-1 w-[96px] rounded-sm text-[14px]'>Signup</Link>
                <Link to={"/admin/dashboard"} className='border-2  border-white px-4 py-1 w-[96px] rounded-sm text-[14px] bg-yellow-600'>Dashboard</Link>
              </div>
            </nav>
