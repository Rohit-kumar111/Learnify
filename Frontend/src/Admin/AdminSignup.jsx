...
const AdminSignup = () => {
  ...
  async function formHandler(e) {
    ...
    try {
      ...
      const response = await axios.post(`${BACKEND_URL}/admin/signup`, {// sending the data
        firstName,
        lastName,
        email,
        password
          ...
          <div className='h-screen flex items-center justify-center text-white max-w-[75vw] mx-auto '>
            ...
            <nav className=' absolute top-0 left-0 w-full flex items-center justify-between py-6 '>
              <div className='flex gap-x-2 ml-4' >
                ...
                <Link className='font-bold text-2xl text-yellow-400'>Courses</Link>
              </div>
              <div className='space-x-4 sm:space-x-2 sm:right-6 relative right-3'>
                <Link to={"/admin/login"} className='border-2 bg-transparent border-white px-4 py-1 w-[5vw] rounded-sm text-[14px]'>Login</Link>
              </div>
            </nav>
